...
class Excel():
    ...
    def create_list(self):
        ...
        for col_idx, question in enumerate(list_quest, start=2):  # Начинаем со 2-й колонки
        ...
        for row_idx, subject in enumerate(list_subj, start=2):  # Начинаем со 2-й строки
            ...
            for col_idx, question in enumerate(list_quest, start=2):
                if(len(self.setts_json['Questions_for_subject'][question]) > 1):
                    ...
                    numeric_scores = [ans for ans in scores if isinstance(ans, (int, float))]
    ...
    def create_teachers_list(self):
        ...
        sheet.column_dimensions['A'].width = len(max(self.csv["Преподаватели"].keys(), key=len))
        ...
        total_values = [sheet.cell(row=row_idx, column=last_col).value for row_idx in range(2, len(list_teachers) + 2) if sheet.cell(row=row_idx, column=last_col).value is not None]
        total_avg = sum(total_values) / len(total_values) if total_values else 0
        sheet.cell(row=avg_row, column=last_col, value=total_avg).alignment = Alignment(horizontal='center')
